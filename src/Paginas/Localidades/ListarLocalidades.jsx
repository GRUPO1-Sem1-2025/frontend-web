...
export default function ListarOmnibus() {
  ...
  const mostrarActivo = (activo) => {
    return (
      <Tag
        value={activo ? "Sí" : "No"}
        severity={activo ? "success" : "danger"}
      />
    );
  };
  ...
  return (
    <>
      ...
      <Card style={{ marginTop: "1rem" }}>
        ...
        >
          <Button
            label="Alta localidad"
            icon="pi pi-plus"
            onClick={() => setMostrarAlta(true)}
          />
          <Button
            label="Carga Masiva localidades"
            icon="pi pi-upload"
            severity="success"
        ...
        >
          ...
          <Column
            ...
            dataType="boolean"
            sortable
            body={(rowData) => mostrarActivo(rowData.activo)}
            showFilterMenu={false}
            filter
            filterElement={ActivoFiltro}
          />
        </DataTable>
      </Card>
      ...
      <AltaLocalidadModal
        visible={mostrarAlta}
        onHide={() => setMostrarAlta(false)}
        ref={toastRef}
        onSuccess={actualizarListaOmnibus}
      />
      <CargaMasivaLocalidadesModal
        visible={mostrarCargaMasiva}
        onHide={() => setMostrarCargaMasiva(false)}
      />
    </>
  );
}
